...
\`\`\`bash
# List all video models for the newapi provider
lh model list newapi --type video

# List only enabled video models
lh model list newapi --type video --enabled

# List image generation models
lh model list newapi --type image
\`\`\`
...
lh gen video "a cat riding a skateboard" -p newapi -m dreamina-seedance-2-0-260128

# ❌ Wrong — guessed slugs will fail with no_valid_channel_error
lh gen video "a cat riding a skateboard" -p newapi -m seedance-2.0
\`\`\`
