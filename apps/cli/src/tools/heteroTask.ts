 ...
 * Tells the agent how to push updates back to the MasterLion user via `lh notify`.
 */
function buildNotifyProtocol(lhPath: string, topicId: string): string {
  return (
    `## Context: This task was dispatched by MasterLion\n\n` +
    `This conversation / task was sent to you by the **MasterLion platform** on behalf of a user. You are running as a background agent; the user is waiting for your response inside the MasterLion chat interface.\n\n` +
...
export async function runHeteroTask(params: RunHeteroTaskParams): Promise<string> {
  ...
  if (agentType === 'openclaw') {
    ...
    // back to the MasterLion UI — even if the previous turn failed and the session
