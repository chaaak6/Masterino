...
vi.mock('../settings', () => ({
  loadSettings: vi.fn().mockReturnValue({ serverUrl: 'https://aihub.bielcrystal.com' }),
  resolveServerUrl: vi.fn(() =>
    (process.env.LOBEHUB_SERVER || 'https://aihub.bielcrystal.com').replace(/\/$/, ''),
  ),
}));
...
describe('resolveToken', () => {
  ...
  describe('with environment api key', () => {
    it('should return API key from environment', async () => {
      ...
      expect(getUserIdFromApiKey).toHaveBeenCalledWith('sk-lh-test', 'https://aihub.bielcrystal.com');
      expect(result).toEqual({
        serverUrl: 'https://aihub.bielcrystal.com',
        token: 'sk-lh-test',
        tokenType: 'apiKey',
