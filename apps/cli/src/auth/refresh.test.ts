...
vi.mock('../settings', () => {
  const loadSettings = vi.fn().mockReturnValue({ serverUrl: 'https://aihub.bielcrystal.com' });

  return {
    loadSettings,
    resolveServerUrl: vi.fn(
      () => loadSettings()?.serverUrl || 'https://aihub.bielcrystal.com',
    ),
  };
});
...
describe('getValidToken', () => {
  beforeEach(() => {
    ...
    vi.mocked(loadSettings).mockReturnValue({ serverUrl: 'https://aihub.bielcrystal.com' });
  });
