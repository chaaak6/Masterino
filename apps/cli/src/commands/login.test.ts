...
vi.mock('../settings', () => ({
  ...
  normalizeUrl: vi.fn((url: string | undefined) => (url ? url.replace(/\/$/, '') : undefined)),
  saveSettings: vi.fn(),
}));
...
describe('login command', () => {
  ...
  function deviceAuthResponse(overrides: Record<string, any> = {}) {
    return {
      json: vi.fn().mockResolvedValue({
        ...
        interval: 1,
        user_code: 'USER-CODE',
        verification_uri: 'https://aihub.bielcrystal.com/verify',
        verification_uri_complete: 'https://aihub.bielcrystal.com/verify?code=USER-CODE',
        ...overrides,
      }),
      ok: true,
    } as any;
  ...
  it('should complete login flow successfully', async () => {
    ...
    expect(saveSettings).toHaveBeenCalledWith({ serverUrl: 'https://aihub.bielcrystal.com' });
  ...
  it('should use environment api key without storing credentials', async () => {
    ...
    expect(getUserIdFromApiKey).toHaveBeenCalledWith('sk-lh-env-test', 'https://aihub.bielcrystal.com');
