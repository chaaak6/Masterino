...
export class SearchService {
  ...
  async webSearch({ query, searchCategories, searchEngines, searchTimeRange }: SearchQuery) {
    ...
    if (lastErrorDetail) {
      return { costTime: 0, errorDetail: lastErrorDetail, query, resultNumbers: 0, results: [] };
    }

    // All providers exhausted without an implementation error, return empty result
