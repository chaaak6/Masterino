...
describe('SearchService', () => {
  ...
  describe('webSearch - provider fallback (turn mode)', () => {
    ...
    it('should return the last provider error when every local search provider is unavailable', async () => {
      const mockImpl1 = { query: vi.fn().mockRejectedValue(new Error('SearXNG is unavailable')) };
      const mockImpl2 = { query: vi.fn().mockRejectedValue(new Error('Fallback is unavailable')) };
      ...
      expect(result).toEqual({
        costTime: 0,
        errorDetail: 'Fallback is unavailable',
        query: 'test',
        resultNumbers: 0,
        results: [],
      });
