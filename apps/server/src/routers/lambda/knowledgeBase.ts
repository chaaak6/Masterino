...
import type { ResourcePermission } from '@/database/repositories/enterprise/resourceAclRepository';
...
const assertKnowledgeBaseAccess = async (
  ...
  knowledgeBase?: Awaited<ReturnType<KnowledgeBaseModel['findById']>>,
) => {
  const row = knowledgeBase ?? (await ctx.knowledgeBaseModel.findById(id));
  ...
  await resourceAclService.assertCan({
    ...
    resource: { resourceId: id, resourceType: 'knowledge_base' },
    userId: ctx.userId,
    workspaceId: ctx.workspaceId ?? null,
  });
...
export const knowledgeBaseRouter = router({
  addFilesToKnowledgeBase: knowledgeBaseProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      await assertKnowledgeBaseAccess(ctx, input.knowledgeBaseId, 'write');
  ...
  getKnowledgeBaseById: knowledgeBaseProcedure
    ...
    .query(async ({ ctx, input }): Promise<KnowledgeBaseItem | undefined> => {
      return assertKnowledgeBaseAccess(ctx, input.id, 'read');
    }),
  ...
  removeKnowledgeBase: knowledgeBaseProcedure
    ...
    .input(z.object({ id: z.string() }))
    .mutation(async ({ input, ctx }) => {
      await assertKnowledgeBaseAccess(ctx, input.id, 'manage');
  ...
  updateKnowledgeBase: knowledgeBaseProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      await assertKnowledgeBaseAccess(ctx, input.id, 'write');
