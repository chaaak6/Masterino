...
const assertDocumentAccess = async (
  ...
  document?: Awaited<ReturnType<DocumentModel['findById']>>,
) => {
  const row = document ?? (await ctx.documentModel.findById(id));
  ...
  await resourceAclService.assertCan({
    ...
    resource: scope.exists ? scope.resource : resource,
...
const assertTargetContainerWrite = async (
  ctx: any,
  input: { knowledgeBaseId?: string; parentId?: null | string },
) => {
  if (input.knowledgeBaseId) {
    const resource = { resourceId: input.knowledgeBaseId, resourceType: 'knowledge_base' as const };
    const scope = await resolveResourceAclScope(ctx.serverDB, resource);
  ...
  if (parent) await assertDocumentAccess(ctx, parent.id, 'write', parent);
};
...
export const documentRouter = router({
  createDocument: documentProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertTargetContainerWrite(ctx, {
  ...
  deleteDocument: documentProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await assertDocumentAccess(ctx, input.id, 'manage');
  ...
  getDocumentById: documentProcedure
    ...
    .query(async ({ ctx, input }) => {
      await assertDocumentAccess(ctx, input.id, 'read');
  ...
  updateDocument: documentProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertDocumentAccess(ctx, id, 'write');
