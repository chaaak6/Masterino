---
title: Connect Masterion to QQ
description: >-
  Learn how to create a QQ bot and connect it to your Masterion agent as a message
...
# Connect Masterion to QQ

By connecting a QQ channel to your Masterion agent, users can interact with the AI assistant through QQ group chats, guild channels, and direct messages.
...
Masterion supports two connection modes for QQ bots:
...
<Steps>
  ...
  If you are using **Webhook mode**, navigate to **Development Settings** → **Callback Configuration** in the QQ Open Platform. You will need to paste the Masterion Callback URL here after completing Step 2.

  If you are using **WebSocket mode** (default), skip this step — no callback URL is needed.
</Steps>

## Step 2: Configure QQ in Masterion

<Steps>
  ### Open Channel Settings

  In Masterion, navigate to your agent's settings, then select the **Channels** tab. Click **QQ** from the platform list.
...
<Steps>
  ...
  Go back to the QQ Open Platform, navigate to **Development Settings** → **Callback Configuration**. Paste the **Callback URL** you copied from Masterion.
  ...
  The QQ Open Platform will send a verification request to your Callback URL. Masterion handles this automatically using Ed25519 signature verification.
</Steps>
...
Click **Test Connection** in Masterion's channel settings to verify the integration. Then open QQ, find your bot, and send a message. The bot should respond through your Masterion agent.
...
To get it: DM the bot once with any message and check the server logs for the `tiny_id` field on the inbound event payload (or read it from the OpenAPI dashboard if available). Paste the long numeric ID into **Your Platform User ID** in Masterion's Advanced Settings.
...
- **Callback URL verification failed (Webhook mode):** Ensure you saved the configuration in Masterion first and the URL was copied correctly. Masterion handles Ed25519 verification automatically.
...
- **Test Connection failed:** Double-check the App ID and App Secret in Masterion's channel settings.
