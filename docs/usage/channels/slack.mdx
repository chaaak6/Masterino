---
title: Connect MasterLion to Slack
description: >-
  Learn how to create a Slack app and connect it to your MasterLion agent as a
...
# Connect MasterLion to Slack

By connecting a Slack channel to your MasterLion agent, users can interact with the AI assistant directly through Slack channels and direct messages.
...
MasterLion supports two connection modes for Slack:
...
<Steps>
  ...
  ```yaml
  display_information:
    name: MasterLion Assistant
    description: AI assistant powered by MasterLion
  features:
    app_home:
    ...
    bot_user:
      display_name: MasterLion Assistant
      always_online: true
    slash_commands:
      - command: /new
...
### Step 3: Configure Slack in MasterLion

<Steps>
  ### Open Channel Settings

  In MasterLion, navigate to your agent's settings → **Channels** tab → click **Slack**.
...
Click **Test Connection** in MasterLion, then go to Slack, invite the bot to a channel, and mention it with `@MasterLion Assistant` to confirm it responds.
...
<Steps>
  ...
  ### Configure in MasterLion

  Enter **Application ID**, **Bot Token**, and **Signing Secret** in MasterLion's Slack channel settings. Set **Connection Mode** to **Webhook** in Advanced Settings. Save and copy the displayed **Webhook URL**.
...
To get it: in Slack, click your avatar → **Profile**, then click the `⋮` overflow menu and choose **Copy member ID**. Member IDs start with `U`. Paste it into **Your Platform User ID** in MasterLion's Advanced Settings.
...
To get it: open Slack in the browser; the URL contains the team ID (`https://app.slack.com/client/T01ABCDEF/...`) — copy the part starting with `T`. Paste it into **Default Server** in MasterLion's Advanced Settings.
...
- **Bot ignores DMs even though Slack is configured correctly:** Open **Advanced Settings** in MasterLion and check **DM Policy**. If it is `Disabled`, switch to `Open` or `Allowlist`. If it is `Allowlist`, confirm the sender's Slack member ID is listed in **Allowed User IDs**.
...
- **`missing_scope` error (e.g., `needed: 'users:read'`) in logs:** Your app is missing a required OAuth scope, or you added a scope but never reinstalled the app. The `users:read` scope is required to resolve `@username` mentions in messages. Go to **OAuth & Permissions** → **Bot Token Scopes**, confirm all scopes listed in the manifest are present, then click **Reinstall to Workspace** at the top of the page and reauthorize. Update the **Bot User OAuth Token** in MasterLion if it changes.
