---
title: Connect Masterion to WeChat
description: >-
  Learn how to connect a WeChat bot to your Masterion agent via the iLink Bot API,
...
# Connect Masterion to WeChat

By connecting a WeChat channel to your Masterion agent, users can interact with the AI assistant through WeChat private chats and group conversations.
...
In Masterion, navigate to your agent's settings, then select the **Channels** tab. Click **WeChat** from the platform list.
...
<Steps>
  ...
  Open WeChat on your phone, go to **Scan** (via the + button in the top right), and scan the QR code displayed in Masterion.
  ...
  Once confirmed, Masterion will automatically save your credentials and connect the bot. You should see a success message in the channel settings.
</Steps>

## Step 3: Test the Bot

Open WeChat, find your bot contact, and send a message. The bot should respond through your Masterion agent.
...
1. When you scan the QR code, Masterion obtains a bot token from WeChat's iLink API
2. Masterion continuously polls the iLink API for new messages (\~35 second intervals)
3. When a message arrives, it is routed through the Masterion agent for processing
