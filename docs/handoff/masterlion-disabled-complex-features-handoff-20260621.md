...
当前 `productFeatures.disabled*` 文案为“敬请期待”，后续建议替换为更明确的 MasterLion 口径，例如“当前聚焦聊天与 Aihub 模型能力，复杂功能正在整理中”。
...
- 明确 MasterLion 内部市场是否需要 agent、skill、mcp、model、provider 全量模块。
- 接入企业可见范围和审核策略。
...
- 明确 Page 工作台在 MasterLion 中的定位：文档编辑、知识沉淀，还是分享页。
- 替换旧品牌视觉和分享图。
