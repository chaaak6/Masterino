# Masterino Enterprise Architecture Plan

更新时间：2026-06-20

## 1. 总体目标

Masterino 要从单一 Web 客户端演进为企业 AI 工作台，核心目标是：
...
建议保持当前 LobeHub/Masterino Web 工程不被后台侵入，新增独立后台项目：
...
```mermaid
flowchart LR
  User["企业用户浏览器"] --> Web["Masterino Web 客户端"]
  AdminUser["企业管理员浏览器"] --> Admin["Masterino Admin 管理后台"]
  Web --> API["apps/server API"]
  Admin --> API
  API --> PG["Masterino Postgres"]
...
sequenceDiagram
  ...
  participant API as Masterino API
