...
本文档汇总 MasterLion 从 v0.0.1 到 v0.0.5 部署过程中遇到的所有坑和解决方案，供后续部署和运维参考。每一条都是实际踩过的，不是理论推测。
...
腾讯云 COS 域名在服务器内网解析到 link-local 地址 `169.254.0.47`，被 MasterLion 的 SSRF 防护拦截。AI 模型在分析图片时需要从 COS 下载图片，这个请求从服务端发出，经过 SSRF 检查。
...
MasterLion 的数据库迁移依赖 `pgvector`（向量存储）和 `pg_search`（全文搜索），普通 PostgreSQL 镜像不带这些扩展。
...
│   ├── PostgreSQL (1Panel-postgresql-bjeC) — 不用于 MasterLion
│   ├── Redis (1Panel-redis-TyNJ) — 不用于 MasterLion
│   └── RustFS (1Panel-rustfs-oROT) — 不用于 MasterLion
│
└── MasterLion 容器 (masterlion-net + 1panel-network)
