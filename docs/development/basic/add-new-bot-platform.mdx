...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to MasterLion's
...
This guide walks through the steps to add a new bot platform to MasterLion's channel system. The platform architecture is modular — each platform is a self-contained directory under `src/server/services/bot/platforms/`.
...
export const myPlatform: PlatformDefinition = {
  ...
  documentation: {
    ...
    setupGuideUrl: 'https://aihub.bielcrystal.com/docs/usage/channels/<platform>',
  },
  schema,
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in MasterLion → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
