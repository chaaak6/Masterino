...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to Masterino's
...
This guide walks through the steps to add a new bot platform to Masterino's channel system. The platform architecture is modular — each platform is a self-contained directory under `src/server/services/bot/platforms/`.
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in Masterino → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
