---
title: MasterLion Feature Development Complete Guide
description: A comprehensive guide for developers on implementing features in MasterLion.
tags:
  - MasterLion
  - Feature Development
  - Developer Guide
...
# MasterLion Feature Development Complete Guide

This document aims to guide developers on how to develop a complete feature in MasterLion.

We will use [RFC 021 - Custom Assistant Opening Guidance](https://github.com/biel-cc/MasterLion/discussions/891) as an example to illustrate the complete implementation process.

## 1. Update Schema

MasterLion uses a PostgreSQL database, with [Drizzle ORM](https://orm.drizzle.team/) to operate the database.
...
After adjusting the schema, you need to generate and optimize migration files. See the [Database Migration Guide](https://github.com/biel-cc/MasterLion/blob/main/.agents/skills/drizzle/references/db-migrations.md) for detailed steps.
...
MasterLion uses [zustand](https://zustand.docs.pmnd.rs/getting-started/introduction) as the global state management framework. For detailed practices on state management, refer to [State Management Best Practices](/docs/development/state-management/state-management-intro).
...
MasterLion is an internationalized project using [react-i18next](https://github.com/i18next/react-i18next). Newly added UI text needs to:
...
The project uses Vitest for unit testing. See the [Testing Skill Guide](https://github.com/biel-cc/MasterLion/blob/main/.agents/skills/testing/SKILL.md) for details.
...
The above is the complete implementation process for the MasterLion opening settings feature, covering the full chain from database schema → data model → Service/Model → Store → i18n → UI → testing. Developers can refer to this document for developing related features.
