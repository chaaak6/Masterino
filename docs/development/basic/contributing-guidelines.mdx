...
description: >-
  Learn about MasterLion's code style and contribution process for consistent
  coding.
tags:
  - Code Style
...
Welcome to the Code Style and Contribution Guidelines for MasterLion. This guide will help you understand our code standards and contribution process, ensuring code consistency and smooth project progression.

## Code Style

In MasterLion, we use the [@lobehub/lint](https://github.com/lobehub/lobe-lint) package to maintain a unified code style. This package incorporates configurations for `ESLint`, `Prettier`, `remarklint`, and `stylelint` to ensure that our JavaScript, Markdown, and CSS files adhere to the same coding standards.
...
MasterLion follows the gitmoji and semantic release as our code submission and release process.
...
MasterLion has comprehensive unit tests (Vitest) and E2E tests (Cucumber + Playwright), which run automatically via GitHub Actions CI on every PR. Before submitting a PR or requesting a merge, make sure all tests pass.
...
Fork [lobehub/lobehub](https://github.com/biel-cc/MasterLion) on GitHub, then clone your fork locally.
...
```bash
git remote add upstream https://github.com/biel-cc/MasterLion.git
git fetch upstream
...
- **Features** — Check [issues labeled `good first issue`](https://github.com/biel-cc/MasterLion/issues?q=is%3Aissue+label%3A%22good+first+issue%22) to find beginner-friendly tasks
