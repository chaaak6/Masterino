---
title: Masterion Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Masterion Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Masterion image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Masterion Container

  Assuming the Masterion container is named `lobehub`, use the following commands to stop and remove the currently running Masterion container:
  ...
  ### Pull the Latest Masterion Image

  Use the following command to pull the latest Docker image for Masterion:
  ...
  Redeploy the Masterion container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Masterion's chat records are stored in your local browser. Therefore, when redeploying Masterion using Docker, your chat records will not be lost.
</Callout>
