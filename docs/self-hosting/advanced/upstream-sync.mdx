---
title: MasterLion Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for MasterLion Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest MasterLion image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running MasterLion Container

  Assuming the MasterLion container is named `lobehub`, use the following commands to stop and remove the currently running MasterLion container:
  ...
  ### Pull the Latest MasterLion Image

  Use the following command to pull the latest Docker image for MasterLion:
  ...
  Redeploy the MasterLion container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of MasterLion's chat records are stored in your local browser. Therefore, when redeploying MasterLion using Docker, your chat records will not be lost.
</Callout>
