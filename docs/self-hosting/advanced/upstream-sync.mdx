---
title: Masterino Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Masterino Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Masterino image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Masterino Container

  Assuming the Masterino container is named `lobehub`, use the following commands to stop and remove the currently running Masterino container:
  ...
  ### Pull the Latest Masterino Image

  Use the following command to pull the latest Docker image for Masterino:
  ...
  Redeploy the Masterino container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Masterino's chat records are stored in your local browser. Therefore, when redeploying Masterino using Docker, your chat records will not be lost.
</Callout>
