...
description: >-
  Learn how to configure Redis cache service to optimize Masterion performance and
  session management.
tags:
  - Redis
  - Cache
...
Masterion uses Redis as a high-performance cache and session storage service to optimize system performance and manage user authentication state.

<Callout type={'info'}>
  Masterion uses the standard Redis protocol (via ioredis library), supporting any Redis
...
Redis is used in Masterion for the following scenarios:
...
<Steps>
  ...
  The prefix for Redis keys, used to isolate Masterion data in a shared Redis instance.
...
<Callout type={'warning'}>
  Redis is an optional service. If `REDIS_URL` is not configured, Masterion will still function
