---
title: Deploy MasterLion with database on Vercel
description: >-
  Learn how to deploy MasterLion with database on Vercel with ease, including:
...
This article will detail how to deploy the server database version of MasterLion on Vercel, including: 1) database configuration; 2) identity authentication service configuration; 3) steps for setting up the S3 storage service.
...
<Steps>
  ...
  Finally, you need to add the `APP_URL` environment variable, which specifies the URL address of the MasterLion application.
</Steps>
...
<Steps>
  ...
  You need to obtain the access key for S3 so that the MasterLion server has permission to access the S3 storage service. In R2, you can configure the access key in the account details:
...
After completing the steps above, the configuration of the server-side database should be done. Next, we can deploy MasterLion to Vercel and then visit your Vercel link to verify if the server-side database is working correctly.
...
To receive future MasterLion updates:

1. **Delete** the current project from Vercel dashboard (Settings → Delete Project)
2. Manually **fork** [lobehub/lobehub](https://github.com/biel-cc/MasterLion) on GitHub
