---
title: Deploying Masterion Database with Docker
description: >-
  Learn how to deploy the Masterion server database version using Docker on Linux
  and local machines.
tags:
  - Masterion
  - Docker
...
Here is the process for deploying the Masterion server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for Masterion to implement RAG and knowledge base search.
  </Callout>
...
The data version of Masterion also supports direct use on a local Mac/Windows machine.
