---
title: Deploying MasterLion Database with Docker
description: >-
  Learn how to deploy the MasterLion server database version using Docker on Linux
  and local machines.
tags:
  - MasterLion
  - Docker
...
Here is the process for deploying the MasterLion server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for MasterLion to implement RAG and knowledge base search.
  </Callout>
...
The data version of MasterLion also supports direct use on a local Mac/Windows machine.
