---
title: Deploying Masterino Database with Docker
description: >-
  Learn how to deploy the Masterino server database version using Docker on Linux
  and local machines.
tags:
  - Masterino
  - Docker
...
Here is the process for deploying the Masterino server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for Masterino to implement RAG and knowledge base search.
  </Callout>
...
The data version of Masterino also supports direct use on a local Mac/Windows machine.
