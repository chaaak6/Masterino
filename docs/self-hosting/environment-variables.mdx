---
title: Masterion Environment Variables - Customizing Guide
description: >-
  Learn how to customize Masterion configuration using environment variables for
...
Masterion provides some additional configuration options when deployed, which can be customized using environment variables.
...
If you need to override `NEXT_PUBLIC` environment variables, you can build a custom Docker image using GitHub Actions without forking the entire Masterion repository. Here's a guide on how to do this:
...
6. To build your custom image, go to the "Actions" tab in your GitHub repository and manually trigger the "Build Custom Masterion Image" workflow.

This approach allows you to create a custom build with your desired `NEXT_PUBLIC` variables without maintaining a full fork of the Masterion repository. You can trigger a new build whenever you need to update your custom image.
