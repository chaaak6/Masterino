...
description: >-
  Guide for migrating your Masterino deployment from NextAuth authentication to
...
This guide helps you migrate your existing NextAuth-based Masterino deployment to Better Auth.

<Callout type={'info'}>
  Better Auth is the recommended authentication solution for Masterino. It offers simpler configuration, more SSO providers, and better self-hosting support.
</Callout>
...
<Callout type={'error'}>
  **Important Notice**:
  ...
  - Masterino is not responsible for any data loss or issues that may occur during the migration process
...
1. Clone the Masterino repository and install dependencies:
...
If you see a redirect to `signin?callbackUrl=...&error=email_not_found`, it means Masterino cannot get the user's email.
...
When configuring SSO connections (e.g., GitHub in Auth0), make sure to enable **Email Address** permission in the Attributes section. Masterino requires user email for authentication.
...
3. The user data will be synced to Masterino via Webhook, and the user can then log in
