...
Masterino 支持使用 Better Auth 配置外部身份验证服务，供企业 / 组织内部使用，统一管理用户授权。
...
要在 Masterino 中启用 Better Auth，请设置以下环境变量：
...
允许 Masterino 在身份提供商中用户信息更新时接收通知。支持的提供商包括 Casdoor 和 Logto。请参考具体提供商文档进行配置。
