...
Masterino has deeply integrated with Clerk to provide users with a secure and convenient login and registration experience.

By setting the environment variables `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` in Masterino's environment, you can enable and use Clerk.
...
Before using NextAuth, please set the following variables in Masterino's environment variables:
