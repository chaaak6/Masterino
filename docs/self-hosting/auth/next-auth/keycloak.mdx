---
title: Configuring Keycloak Authentication Service in MasterLion
description: >-
  Learn how to configure the Keycloak authentication service in MasterLion,
...
# Configuring Keycloak Authentication Service in MasterLion
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with MasterLion via Docker
...
- Your MasterLion database version IP/port is `http://LobeHub_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your MasterLion database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "MasterLion", then click "Create"

  2. Create a Client

     - Select "Clients" from the left menu, then click "Create client"
     - Fill in the following information:
       - Client ID: `MasterLion`
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your MasterLion domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the MasterLion environment variables.

  Configure the MasterLion environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/MasterLion` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/MasterLion` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/MasterLion` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for MasterLion. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use MasterLion.
</Callout>
