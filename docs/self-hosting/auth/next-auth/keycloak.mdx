---
title: Configuring Keycloak Authentication Service in Masterion
description: >-
  Learn how to configure the Keycloak authentication service in Masterion,
...
# Configuring Keycloak Authentication Service in Masterion
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Masterion via Docker
...
- Your Masterion database version IP/port is `http://LobeHub_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Masterion database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Masterion", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your Masterion domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Masterion environment variables.

  Configure the Masterion environment variable `AUTH_KEYCLOAK_ISSUER` as:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Masterion. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Masterion.
</Callout>
