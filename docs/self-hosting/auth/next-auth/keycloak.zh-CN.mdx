---
title: 在 Masterion 中配置 Keycloak 身份验证服务
description: 学习如何在 Masterion 中配置 Keycloak 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Keycloak 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'tip'}>
  若你想要私有部署 Keycloak，我们建议你将之与 Masterion 一同使用 Docker Compose
  部署，这样可以更方便地管理服务。
</Callout>
...
- 你的 Masterion IP / 端口为 `http://LobeHub_IP:3210`。
- 你私有部署 Keycloak，其域名为 `http://KEYCLOAK_IP:8080`。

若你使用公网部署，下文假设：

- 你的 Masterion 域名为 `https://lobe.example.com`。
...
<Steps>
  ...
  2. 创建客户端（Client）
     ...
     - 在 "Login settings" 页面：
       ...
       - Web origins: 添加你的 Masterion 域名或 IP
       - 点击 "Save"
  ...
  将获取到的客户端 ID 和客户端密钥，设为 Masterion 环境变量中的 `AUTH_KEYCLOAK_ID` 和 `AUTH_KEYCLOAK_SECRET`。

  配置 Masterion 环境变量中 `AUTH_KEYCLOAK_ISSUER` 为：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Keycloak 身份认证并使用 Masterion。</Callout>
