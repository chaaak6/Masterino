---
title: Configuring Casdoor Authentication for MasterLion
description: >-
  Learn how to configure Casdoor SSO for MasterLion, including creating an
...
<Steps>
  ...
  When deploying MasterLion, you need to configure the following environment variables:

  | Environment Variable     | Type     | Description                                                                   |
  | ------------------------ | -------- | ----------------------------------------------------------------------------- |
  | `AUTH_SECRET`            | Required | Key used to encrypt session tokens. Generate using: `openssl rand -base64 32` |
  | `AUTH_SSO_PROVIDERS`     | Required | SSO provider for MasterLion. Use `casdoor` for Casdoor                           |
  ...
  Configure Casdoor [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) to sync user data updates to MasterLion.
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Casdoor and use MasterLion.
</Callout>

## Docker Compose Deployment with Casdoor

If you're deploying MasterLion using Docker Compose, refer to the following configuration to integrate Casdoor as an authentication service.
...
<Callout type="important">
  ...
  See [https://github.com/biel-cc/MasterLion/discussions/5986](https://github.com/biel-cc/MasterLion/discussions/5986)
</Callout>

### Required Configuration

1. MasterLion needs to communicate with Casdoor, so you need to configure Casdoor's Issuer:

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration affects MasterLion's login authentication service. Ensure the Casdoor service URL is correct.

2. Allow callback URL in Casdoor to point to MasterLion:
...
Cause: MasterLion cannot access the authentication service.

Solution:

- Check if your authentication service is running properly and if MasterLion's network can reach it.

- Direct troubleshooting: Use `curl` in the MasterLion container terminal to access `https://auth.example.com/.well-known/openid-configuration`. If JSON data is returned, your authentication service is working correctly.
...
Casdoor Docker Compose configuration files can be found in the [docker-compose/local/casdoor](https://github.com/biel-cc/MasterLion/tree/main/docker-compose/local/casdoor) directory.
