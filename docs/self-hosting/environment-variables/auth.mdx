---
title: MasterLion Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication
  services in MasterLion, including Better Auth, OAuth SSO, and provider-specific
  details.
tags:
...
MasterLion provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in MasterLion.
...
- Type: Optional
- Description: Set to `1` to block new users from signing up through email/password, Magic Link, or Email OTP while preserving email login and password reset for existing users. OAuth and SSO providers may still create users on their first successful login. `AUTH_DISABLE_EMAIL_PASSWORD=1` also implies this restriction.
- Default: `0`
- Example: `1`
