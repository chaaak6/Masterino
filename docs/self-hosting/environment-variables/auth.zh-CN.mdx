---
title: MasterLion 身份验证服务设置
description: 了解如何配置 MasterLion 的身份验证服务环境变量，包括 Better Auth、OAuth SSO 等。
tags:
  - MasterLion
  - 身份验证服务
  - Better Auth
...
MasterLion 在部署时提供了完善的身份验证服务能力，以下是相关的环境变量，你可以使用这些环境变量轻松定义需要在 MasterLion 中开启的身份验证服务。
...
- 类型：可选
- 描述：设置为 `1` 后，禁止通过邮箱密码、Magic Link 或 Email OTP 创建新用户，同时保留已有用户的邮箱登录和密码重置。OAuth/SSO 首次登录仍可创建用户。`AUTH_DISABLE_EMAIL_PASSWORD=1` 时也会隐含启用此限制。
- 默认值：`0`
- 示例：`1`
