...
- Type: Optional
- Description: Market Trusted Client ID used by the Masterion server to call Market capabilities on behalf of the current user. It must be registered by the target Market service.
- Default: -
...
- Type: Optional
- Description: Lifetime of each Onlyboxes JIT token minted by Masterion.
- Default: `1800`
- Example: `900`
...
If that Market service requires the Masterion server to call sandbox, credential, or skill capabilities on behalf of the current user, also configure Trusted Client credentials:
...
The configured Onlyboxes worker should expose `terminalExec` and `terminalResource`. Masterion uses `terminalExec` as the compatibility layer for shell commands, code execution, and file operations, and uses `terminalResource` for file export through a pre-signed upload URL.
...
<Callout type={'info'}>
  File export still writes the exported artifact to the configured Masterion S3 storage. Configure the
