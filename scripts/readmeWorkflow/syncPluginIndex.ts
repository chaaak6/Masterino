    ...
    .map((item) => [
      [
        genLink(
          ...
          urlJoin('https://aihub.bielcrystal.com/discover/plugin', item.identifier),
        ),
...
const runPluginTable = async (lang?: string) => {
  ...
  const newMd = updateReadme(
    ...
    [
      mdTable,
      `> 📊 Total plugins: ${genLink(`<kbd>**${data.length}**</kbd>`, 'https://aihub.bielcrystal.com/discover/plugins')}`,
    ].join('\n\n'),
  );
