    ...
    .map((item) => [
      [
        genLink(
          ...
          urlJoin('https://aihub.bielcrystal.com/discover/assistant', item.identifier),
        ),
...
const runAgentTable = async (lang?: string) => {
  ...
  const newMd = updateReadme(
    ...
    [
      mdTable,
      `> 📊 Total agents: ${genLink(`<kbd>**${data.length}**</kbd> `, 'https://aihub.bielcrystal.com/discover/assistants')}`,
    ].join('\n\n'),
  );
