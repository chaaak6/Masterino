...
const ImageUrlsUpload = memo(() => {
  ...
  if (maxCount === 1) {
    const handleSingleChange = (
      data?:
        | string // Legacy payload: just URL
        | { dimensions?: { height: number; width: number }; url: string }, // URL payload with dimensions
    ) => {
  ...
  const handleChange = (
    data:
      | string[] // Legacy payload: just URLs
      | { dimensions?: { height: number; width: number }; urls: string[] }, // URL payload with first image dimensions
  ) => {
