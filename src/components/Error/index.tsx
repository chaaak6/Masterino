...
const fallbackText = {
  backHome: '返回首页',
  desc: '抱歉，页面遇到了一些问题。请刷新页面或返回首页重试。',
  retry: '重试',
  stack: '错误堆栈',
  title: '页面暂时不可用',
};
  ...
  return (
    <Flexbox align={'center'} justify={'center'} style={{ minHeight: '100dvh', width: '100%' }}>
      ...
      <h2 style={{ fontWeight: 'bold', marginTop: '1em', textAlign: 'center' }}>
        {t('error.title', { defaultValue: fallbackText.title })}
      </h2>
      <p style={{ marginBottom: '2em' }}>
        {t('error.desc', { defaultValue: fallbackText.desc })}
      </p>
      <Flexbox horizontal gap={12} style={{ marginBottom: '2em' }}>
        <Button onClick={() => window.location.reload()}>
          {t('error.retry', { defaultValue: fallbackText.retry })}
        </Button>
        <Button type={'primary'} onClick={() => (window.location.href = resetPath)}>
          {t('error.backHome', { defaultValue: fallbackText.backHome })}
        </Button>
      </Flexbox>
      {hasStack && (
        <Block
        ...
        >
          ...
          >
            <AccordionItem
              ...
              itemKey={'stack'}
              title={t('error.stack', { defaultValue: fallbackText.stack })}
            >
