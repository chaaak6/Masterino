...
const isOfficialAuthor = (author?: string) => author === 'MasterLion' || author === 'LobeHub';

const officialTag = (
  <Tooltip placement={'top'} title={'MasterLion'}>
        ...
        ? COMPOSIO_APP_TYPES.filter(
            ...
            if (server?.status === ComposioServerStatus.ACTIVE) {
              return createManagedSkillItem({
                ...
                extraTag: isOfficialAuthor(type.author) ? officialTag : undefined,
                icon,
      ...
      isLobehubSkillEnabled
        ? LOBEHUB_SKILL_PROVIDERS.filter(
            ...
            if (server?.status === LobehubSkillStatus.CONNECTED || server?.isConnected) {
              return createManagedSkillItem({
                badge: <Icon icon={McpIcon} size={12} />,
                extraTag: isOfficialAuthor(provider.author) ? officialTag : undefined,
                icon,
                id: server.identifier,
                popoverContent,
