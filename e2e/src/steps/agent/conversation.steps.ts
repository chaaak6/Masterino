...
Given('用户进入 小宗狮AI 对话页面', { timeout: 30_000 }, async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 小宗狮AI...');
  // Find and click on the default Masterino agent in the sidebar/home.
  const defaultAgent = this.page.getByText('小宗狮AI', { exact: true }).first();
  await expect(defaultAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 小宗狮AI...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain the agent title.
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.getByText('小宗狮AI', { exact: true }),
  });
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /小宗狮AI|AI/ }),
  });
