...
BeforeAll({ timeout: 600_000 }, async function () {
  ...
  if (process.env.SKIP_E2E_LOGIN === '1') {
    console.log('Skipping one-time login because SKIP_E2E_LOGIN=1');
    return;
  }
  ...
  if (process.env.PLAYWRIGHT_CHANNEL) {
    launchOptions.channel = process.env.PLAYWRIGHT_CHANNEL as 'chrome' | 'msedge';
  }
