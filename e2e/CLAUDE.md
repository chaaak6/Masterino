...
本文档记录了在 MasterLion E2E 测试开发中的经验和最佳实践。

Related: historical E2E baseline work item `LOBE-2417`.
...
MasterLion 使用特定的 SSE 格式，必须严格匹配：
...
```typescript
// 为特定用户消息设置响应
llmMockManager.setResponse('你好', '你好！我是 MasterLion，有什么可以帮助你的？');
...
MasterLion 当前仍使用 legacy `@lobehub/editor` 包作为聊天输入框，是一个 contenteditable 的富文本编辑器。
